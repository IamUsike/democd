...
**Tester:** shreya (QA-1) · support sathwik (deploy / Actuator)  
**Client:** Windows VM (k6) → Linux VM API `http://10.9.69.3:8081`  
**App deploy:** Docker Compose on single box (JVM + MySQL co-located)

**Also see:** [`LOAD_TEST_GUIDE.md`](./LOAD_TEST_GUIDE.md) (runbook) · [`PRESENTATION_SCRIPT.md`](./PRESENTATION_SCRIPT.md) · Cursor canvas `load-test-results.canvas.tsx` (local IDE only)
...
| Seed row count before tests | ~200k–300k (SQL seed; see guide) |
| k6 version | Windows client (winget `k6.k6`) |
...
- JVM: contended with mysqld on shared cores (see raw `top` snippets below)
- mysqld: high write / commit share under soak
- Swap (`vmstat` si/so): present on host (~3.9 GiB swap configured)

Interpretation: Short ramps (Pass 1–2) stayed healthy (0% fail). Sustained 140 VUs for 10 minutes is where co-location shows — p95 and `rule.evaluate` mean rose together.

What we’d change with a second VM (separate DB host): Move MySQL off the app box, then async queue between ingest and evaluate; re-run Pass 3 at 140 VUs.

## Index verification (EXPLAIN)

| Query | Index used (`key`) | Rows examined (`rows`) | Notes |
|-------|--------------------|------------------------|-------|
| `account_id` + `timestamp` (velocity-style) | `idx_txn_account_timestamp` | 1 | `type=range`, Using index condition |
| `account_id` + `payee_id` (new-payee-style) | `idx_txn_account_payee` | 1 | `type=ref`, Using index |
| `source_type` + `source_id` + `timestamp` ORDER BY | `idx_txn_source_timestamp` | — | Backward index scan (see raw EXPLAIN below) |

Paste `EXPLAIN` output (or screenshots) under each if useful. Full paste is in **Outputs** below.
...
|-------------------|------------|----------|----------|------------|-------|
| →200 (summary) | **234** | **763** | (see raw) | **0%** | 63,376 reqs; no cliff on short ramp |

Where p95 started climbing hard: no hard cliff on short ramp through 200 VUs; soak is where pain shows.  
Observed JVM vs MySQL at that point: both busy; indexes used (not missing-index story).
...
| p95 | **623 ms** |
| error rate | **0%** |
| Notes (GET vs POST) | Always filtered `?accountId=`; reads did not hurt writes |
...
| Duration | 10m |
| p95 | **1.13 s** |
| error rate | **0.09%** (127 / 128,470 timeouts) |
| Swap activity | Host has swap configured; soak stressed shared box |
| Memory trend | Contended JVM + MySQL |
| PROCESSLIST / locks | See raw InnoDB / processlist snippets below |

## rule.evaluate (Actuator)

Sample during / after peak (from `/actuator/metrics/rule.evaluate`):

| Checkpoint | Mean (total/count) |
|------------|--------------------|
| After P1 | ~57 ms |
| After P2 | ~53 ms |
| Mid soak | ~91 ms |
...
Compare to k6 `http_req_duration` p95: Pass 1–2 p95 600–800 ms while rule mean ~50–60 ms → most latency **outside** the timer (DB write, pool, network, alerts). Under soak both rose → shared-box contention.

## Conclusions

1. Max sustainable write RPS (before p95 cliff): **~230+ RPS** on short ramp; sustained soak at 140 VUs drops to **~214 RPS** with p95 **1.13 s**
2. Indexes confirmed via EXPLAIN: **yes**
3. Dominant bottleneck under load: **co-location contention** (JVM + MySQL), not missing indexes
4. Evidence: EXPLAIN keys used; Pass 1–2 0% fail; Pass 3 0.09% timeouts; `rule.evaluate` mean 53→126 ms under soak
5. Phase 3 implications (queue / extract rule workers / separate DB): **separate MySQL VM first**, then **async queue** ingest→evaluate; re-run Pass 3

## Inter-VM connectivity note (ops)

| Source | Destination (Linux app on `:8081`) | Result |
|--------|--------------------------------------|--------|
| Windows VM | Linux VM | Works |
| Linux VM A | Linux VM B | Does **not** work |
| `nc -lvnp 8081` on Linux B ← Windows | | Works |
| `nc -lvnp 8081` on Linux B ← Linux A | | Does **not** work |

Not an application bug — plain `nc` fails Linux→Linux the same way. Treat as security-group / firewall / routing between Linux VMs before blaming Spring/Docker.

## Raw artifacts

- k6 summary JSON / screenshots: pasted under **Outputs**
- `mpstat` / `vmstat` / `iostat` snippets: see campaign notes / VM session
- `SHOW PROCESSLIST` / `SHOW ENGINE INNODB STATUS` snippets: pasted under **Outputs**
