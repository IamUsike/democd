...
investigation). Supports filtering, sorting, pagination, and delta polling
via `afterId`.
...
| q | No | Case-insensitive search across id/source/account/payee/description |
| from / to | No | Timestamp range (ISO-8601) |
| afterId | No | Return only rows with `transactionId > afterId` (live delta poll) |
| page | No | 0-based page index (default `0`) |
| size | No | Page size (default `50`, max `200`) |
| sort | No | `property,direction` — allowed: `timestamp`, `transactionId`, `amount`, `status`. Default `timestamp,desc`; when `afterId` is set and sort omitted, defaults to `transactionId,asc` |
...
- Prefer `afterId` for auto-refresh feeds so the UI does not re-download the full page.
- Path-based helpers (`/account/{id}`, `/status/{status}`, `/type/{type}`) still return full lists and are not paginated.
---
...
Retrieves a paginated list of alerts for the operator UI. By default only
**active** alerts are returned (`OPEN`, `ACKNOWLEDGED`, `INVESTIGATING`).
List items are summaries (no linked transaction IDs); use `GET /alerts/{id}`
for detail + triggering transactions.
...
| sourceType | No | `BANK` or `MERCHANT` |
| sourceId | No | Filter to one source |
| status | No | Single status, or `ALL` for history. **Omitted = active statuses only** |
| severity | No | `HIGH` / `MEDIUM` / `LOW` |
| accountId | No | Filter by account |
| q | No | Search account / source / rule type |
| createdFrom / createdTo | No | Created-at range (ISO-8601) |
| page | No | 0-based page index (default `0`) |
| size | No | Page size (default `50`, max `200`) |
| sort | No | `property,direction` — allowed: `createdAt`, `severity`, `status`, `alertId`. Default `createdAt,desc` |
...
- Default status filter keeps the operator queue on open work, not closed history.
- Pass `status=ALL` (or a specific closed status) for alert history views.
- Linked `transactionIds` are loaded only on `GET /api/v1/alerts/{alertId}`.
