...
> This is the original project brief as given by the instructors, kept
> verbatim for reference. See `Project_milestones.md` for the team's
> actual current scope and status, and `AGENTS.md` / `.cursor/rules/`
> for how we've translated this into build conventions. If this
> document and `Project_milestones.md` ever disagree on scope, the
> milestones file reflects what the team actually decided — this file
> is the historical source, not the live plan.
